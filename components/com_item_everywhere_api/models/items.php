...
class Item_everywhere_apiModelItems extends JModelList
{
    ...
    static function getByIds($ids,$last=0){
        ...
        $query="SELECT DISTINCT jzi.id,jzi.name,jzi.alias,
                ...
                FROM #__zoo_item jzi
                WHERE jzi.application_id=1 AND state=1 AND (publish_down IS NULL OR publish_down>sysdate()) AND jzi.id in (".implode(',', $ids).") AND jzi.id>".$last." ORDER BY jzi.id LIMIT 10;";
